...
function HeroDemo:Init()
	...
	CustomGameEventManager:RegisterListener( "SwapHeroButtonPressed", function(...) return self:OnSwapHeroButtonPressed( ... ) end )
	...
	CustomNetTables:SetTableValue( "demo_table", "ui_defaults", 
		{ 
...
function HeroDemo:Think_InitializePlayerHero( hPlayerHero )
	...
	if hPlayerHero:GetPlayerOwner() then
	...
	if self.m_bInvulnerabilityEnabled then
		...
		for _, hUnit in pairs( hAllPlayerUnits ) do
			hUnit:AddNewModifier( hPlayerHero, nil, "demo_no_damage", nil )
		end
	end
...
-- ToggleDayNight
--------------------------------------------------------------------------------
function HeroDemo:OnToggleDayNight( eventSourceIndex, data )
...
--------------------------------------------------------------------------------
-- ButtonEvent: SwapHeroButtonPressed
--------------------------------------------------------------------------------
function HeroDemo:OnSwapHeroButtonPressed( eventSourceIndex, data )
	local hPlayerHero = PlayerResource:GetSelectedHeroEntity( data.PlayerID )
	if hPlayerHero == nil then
		return
	end
	...
	PrecacheUnitByNameAsync(sHeroToSpawn, function()
		local old_hero = PlayerResource:GetSelectedHeroEntity(data.PlayerID)
		local hero = PlayerResource:ReplaceHeroWith(data.PlayerID, sHeroToSpawn, 99999, 0)
	...
	CustomGameEventManager:Send_ServerToAllClients( "set_new_swapped_hero", {hero_name = sHeroToSpawn} )
...
--------------------------------------------------------------------------------
function HeroDemo:OnSpawnRuneShieldPressed( eventSourceIndex, data )
	local hPlayerHero = PlayerResource:GetSelectedHeroEntity( data.PlayerID )
	if hPlayerHero == nil then
		return
	end
